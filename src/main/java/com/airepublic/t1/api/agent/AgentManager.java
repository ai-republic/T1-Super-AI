...
public class AgentManager {
    ...
    /**
     * Reload agents from the current team workspace.
     * This clears all agents from memory and loads agents from the current team's directory.
     */
    public void reloadAgentsForCurrentTeam(AgentOrchestrator orchestrator) {
        log.info("🔄 Reloading agents for current team workspace...");

        // Stop and clear all existing agents
        for (Agent agent : agents.values()) {
            try {
                agent.stop();
            } catch (Exception e) {
                log.warn("Error stopping agent: {}", agent.getName(), e);
            }
        }
        agents.clear();
        currentAgentName = null;

        // Load agents from the new team workspace
        loadSavedAgents(orchestrator);

        // Set first available agent as current, or null if no agents
        if (!agents.isEmpty()) {
            // Try to get default agent from USER.md first
            try {
                AgentConfigService.UserInfo userInfo = agentConfigService.readUserInfo();
                if (userInfo.defaultAgent != null && !userInfo.defaultAgent.isEmpty() && agents.containsKey(userInfo.defaultAgent)) {
                    currentAgentName = userInfo.defaultAgent;
                    log.info("✅ Set current agent to default from USER.md: {}", currentAgentName);
                } else {
                    // Use first available agent
                    currentAgentName = agents.keySet().iterator().next();
                    log.info("✅ Set current agent to first available: {}", currentAgentName);
                }
            } catch (Exception e) {
            ...
            log.warn("⚠️ No agents available in current team workspace");
        }

        log.info("✅ Agents reloaded for current team");
    }
