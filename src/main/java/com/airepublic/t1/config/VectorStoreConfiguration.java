 ...
 * Requires: agent configuration with defaultProvider set
 */
@Slf4j
@Configuration
...
public class VectorStoreConfiguration {
     ...
     * Only created if configuration is set up (defaultProvider is not null).
     */
    @Bean
    @Conditional(DefaultProviderConfiguredCondition.class)
    public EmbeddingModel embeddingModel(AgentConfigurationManager configManager) {
        var config = configManager.getConfiguration();
        var defaultProvider = config.getFallbackProvider();
