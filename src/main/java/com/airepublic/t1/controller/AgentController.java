...
public class AgentController {
    ...
    private final com.airepublic.t1.config.WorkspaceInitializer workspaceInitializer;
    ...
    public ResponseEntity<com.airepublic.t1.api.dto.ApiResponse<List<AgentInfo>>> listAgents() {
        try {
            ...
            final String currentTeamName = workspaceInitializer.getTeamName();
    ...
    public ResponseEntity<com.airepublic.t1.api.dto.ApiResponse<AgentInfo>> createAgent(
        ...
        try {
            ...
            final com.airepublic.t1.model.AgentConfiguration.LLMProvider defaultProvider =
                    request.getProvider() != null ? request.getProvider() : globalConfig.getFallbackProvider();
    ...
    public ResponseEntity<com.airepublic.t1.api.dto.ApiResponse<Void>> removeAgent(
        ...
        try {
            ...
            if (removed) {
                // Delete agent folder and files from disk
                final boolean folderDeleted = agentConfigService.deleteAgentConfig(name);

                if (folderDeleted) {
                    log.info("Agent '{}' removed from memory and disk", name);
                    return ResponseEntity.ok(com.airepublic.t1.api.dto.ApiResponse.success(
                            "Agent '" + name + "' removed successfully", null));
                } else {
                    log.warn("Agent '{}' removed from memory but folder deletion failed", name);
                    return ResponseEntity.ok(com.airepublic.t1.api.dto.ApiResponse.success(
                            "Agent '" + name + "' removed from memory, but folder deletion failed", null));
                }
            } else {
                return ResponseEntity.badRequest()
    ...
    public ResponseEntity<com.airepublic.t1.api.dto.ApiResponse<AgentInfo>> updateAgent(
        ...
        try {
            ...
            final boolean isRenaming = newName != null && !newName.trim().isEmpty() && !newName.equals(name);

            // If renaming, validate new name doesn't already exist
            if (isRenaming && agentManager.hasAgent(newName)) {
                return ResponseEntity.badRequest()
                        .body(com.airepublic.t1.api.dto.ApiResponse.error("Agent '" + newName + "' already exists"));
            }
            ...
            String finalAgentName = name;
            if (isRenaming) {
                // Rename the agent folder and update CHARACTER.md
                agentConfigService.renameAgent(name, newName);
                finalAgentName = newName;

                // Update agent manager to reflect new name
                agentManager.removeAgent(name);
                final Agent renamedAgent = agentManager.createAgent(newName, orchestrator, config);

                log.info("Agent renamed from '{}' to '{}'", name, newName);
            } else {
                // Just update the configuration without renaming
            ...
            // Get the updated agent (with potentially new name)
            final Agent updatedAgent = agentManager.getAgent(finalAgentName);
            ...
            final String successMessage = isRenaming ?
                    "Agent renamed from '" + name + "' to '" + newName + "' and configuration updated successfully" :
                    "Agent '" + name + "' configuration updated successfully";

            return ResponseEntity.ok(com.airepublic.t1.api.dto.ApiResponse.success(successMessage, agentInfo));
    ...
    @Operation(
            summary = "Clone agent to another team",
            description = "Clones an agent from one team to another by copying the agent folder and updating CHARACTER.md paths"
            )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Agent cloned successfully"),
            @ApiResponse(responseCode = "400", description = "Bad request - validation failed"),
            @ApiResponse(responseCode = "404", description = "Source agent or team not found"),
            @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    @PostMapping("/clone")
    public ResponseEntity<com.airepublic.t1.api.dto.ApiResponse<Void>> cloneAgent(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "Clone agent request with source and target team information",
                    required = true,
                    content = @Content(schema = @Schema(implementation = com.airepublic.t1.api.dto.CloneAgentRequest.class))
                    )
            @RequestBody final com.airepublic.t1.api.dto.CloneAgentRequest request) {
        try {
            if (request.getAgentName() == null || request.getAgentName().trim().isEmpty()) {
                return ResponseEntity.badRequest()
                        .body(com.airepublic.t1.api.dto.ApiResponse.error("Agent name is required"));
            ...
            if (request.getSourceTeam() == null || request.getSourceTeam().trim().isEmpty()) {
                return ResponseEntity.badRequest()
                        .body(com.airepublic.t1.api.dto.ApiResponse.error("Source team is required"));
            }

            if (request.getTargetTeam() == null || request.getTargetTeam().trim().isEmpty()) {
                return ResponseEntity.badRequest()
                        .body(com.airepublic.t1.api.dto.ApiResponse.error("Target team is required"));
            }

            if (request.getSourceTeam().equals(request.getTargetTeam())) {
                return ResponseEntity.badRequest()
                        .body(com.airepublic.t1.api.dto.ApiResponse.error("Source and target teams cannot be the same"));
            }

            // Clone the agent
            agentConfigService.cloneAgentToTeam(request.getAgentName(), request.getSourceTeam(), request.getTargetTeam());

            return ResponseEntity.ok(com.airepublic.t1.api.dto.ApiResponse.success(
                    "Agent '" + request.getAgentName() + "' cloned from team '" + request.getSourceTeam() +
                    "' to team '" + request.getTargetTeam() + "' successfully", null));

        } catch (final java.io.IOException e) {
            log.error("IO error cloning agent", e);
            return ResponseEntity.internalServerError()
                    .body(com.airepublic.t1.api.dto.ApiResponse.error("Failed to clone agent: " + e.getMessage()));
        } catch (final Exception e) {
            log.error("Error cloning agent", e);
            return ResponseEntity.internalServerError()
                    .body(com.airepublic.t1.api.dto.ApiResponse.error("Error cloning agent: " + e.getMessage()));
        }
    }
