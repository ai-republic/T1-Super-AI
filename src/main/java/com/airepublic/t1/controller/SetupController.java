...
public class SetupController {
    ...
    public ResponseEntity<ApiResponse<Void>> completeSetup(@RequestBody final WizardSetupRequest request) {
        try {
            ...
            // Step 1: Configure task-specific models and extract provider configs
            log.info("🔧 Configuring task models...");

            final AgentConfiguration config = configManager.getConfiguration();
            boolean hasGeneralKnowledge = false;
            final Map<LLMProvider, AgentConfiguration.LLMConfig> providerConfigs = new HashMap<>();

            // Configure task-specific models and extract provider configs
            if (request.getTaskModels() != null && !request.getTaskModels().isEmpty()) {
                ...
                for (final Map.Entry<String, WizardSetupRequest.TaskModelConfig> entry : request.getTaskModels().entrySet()) {
                    ...
                    if (taskModelConfig.getProvider() != null && !taskModelConfig.getProvider().isEmpty()) {
                        try {
                            ...
                            // Set task-specific API key if provided
                            if (taskModelConfig.getApiKey() != null && !taskModelConfig.getApiKey().isEmpty()) {
                                taskConfig.setApiKey(taskModelConfig.getApiKey());
                            }
                            // Set task-specific base URL if provided
                            if (taskModelConfig.getBaseUrl() != null && !taskModelConfig.getBaseUrl().isEmpty()) {
                                taskConfig.setBaseUrl(taskModelConfig.getBaseUrl());
                            }

                            config.getTaskModels().put(taskType, taskConfig);
                            taskModelsConfigured++;

                            // Track if GENERAL_KNOWLEDGE is configured
                            if (taskType == AgentConfiguration.TaskType.GENERAL_KNOWLEDGE) {
                                hasGeneralKnowledge = true;
                            }

                            // Extract provider config from task model (for provider registry)
                            if (!providerConfigs.containsKey(taskProvider)) {
                                final AgentConfiguration.LLMConfig llmConfig = new AgentConfiguration.LLMConfig();

                                // Use task model's API key and base URL as defaults for the provider
                                if (taskModelConfig.getApiKey() != null && !taskModelConfig.getApiKey().isEmpty()) {
                                    llmConfig.setApiKey(taskModelConfig.getApiKey());
                                }
                                if (taskModelConfig.getBaseUrl() != null && !taskModelConfig.getBaseUrl().isEmpty()) {
                                    llmConfig.setBaseUrl(taskModelConfig.getBaseUrl());
                                } else {
                                    // Set default base URLs
                                    llmConfig.setBaseUrl(getDefaultBaseUrl(taskProvider));
                                }
                                if (taskModelConfig.getModel() != null && !taskModelConfig.getModel().isEmpty()) {
                                    llmConfig.setModel(taskModelConfig.getModel());
                                }

                                providerConfigs.put(taskProvider, llmConfig);
                                log.info("✅ Extracted provider config for {}", taskProvider);
                            }

                            log.info("✅ Configured task model for {}: {} / {}",
                ...
                // Apply extracted provider configs
                config.getLlmConfigs().putAll(providerConfigs);

                log.info("✅ Task models configured: {}", taskModelsConfigured);
                log.info("✅ Providers configured: {}", providerConfigs.size());
            }

            // Validate GENERAL_KNOWLEDGE is configured
            if (!hasGeneralKnowledge) {
                return ResponseEntity.badRequest()
                        .body(ApiResponse.error("GENERAL_KNOWLEDGE model must be configured (serves as fallback for all tasks)"));
            }

            // Save configuration
            configManager.saveConfiguration();

            // Step 2: Configure team-based workspace (use "Default" if not specified)
            String teamName = (request.getTeamName() != null && !request.getTeamName().trim().isEmpty())
                ? request.getTeamName()
                : "Default";

            // Validate and sanitize team name for filesystem safety
            if (!com.airepublic.t1.util.PathUtils.isValidTeamName(teamName)) {
                log.warn("Team name '{}' contains invalid characters, sanitizing...", teamName);
                teamName = com.airepublic.t1.util.PathUtils.sanitizeTeamName(teamName);
                log.info("Sanitized team name: '{}'", teamName);
            }

            log.info("🏢 Configuring team-based workspace for team: {}", teamName);
            workspaceInitializer.setTeamName(teamName);

            // Step 3: Build responses map for hatching service
            ...
            // Team data (always set, defaults to "Default")
            responses.put("team_name", teamName);
            ...
            // Step 4: Complete the hatch process
            hatchingService.completeHatchProcess(responses);

            log.info("🎉 Wizard setup completed successfully!");
            log.info("   Fallback Model: GENERAL_KNOWLEDGE");
            log.info("   Configured Providers: {}", providerConfigs.size());
            log.info("   Task Models: {}", config.getTaskModels().size());
            log.info("   Team: {}", teamName);
            log.info("   Workspace: ~/.t1-super-ai/workspaces/{}/", teamName);
     ...
     */
    private String getDefaultBaseUrl(LLMProvider provider) {
